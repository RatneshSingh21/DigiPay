...
const ImportWorkLocations = ({ onClose, fetchLocations = () => {} }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        toast.success("Work Locations imported successfully!");
        setFileName(null);
  ...
  const handleDownloadSample = async () => {
    try {
      const response = await axiosInstance.get("/WorkLocation/export", {
        responseType: "blob",
      });
      ...
      link.href = url;
      link.setAttribute("download", "WorkLocations.xlsx");
    ...
    <div className="fixed inset-0 bg-opacity-30 backdrop-blur-sm z-50 flex items-center justify-center px-4">
      ...
      >
        ...
        <h2 className="text-2xl font-semibold text-gray-800 mb-6">
          Work Locations - Upload Excel File
        </h2>
        ...
        <div className="flex items-center gap-4">
          <button
            className="bg-primary text-white cursor-pointer px-6 py-2 rounded-md hover:bg-secondary transition duration-200 flex items-center gap-2"
          ...
          <button
            className="border px-6 py-2 rounded-md cursor-pointer text-gray-700 hover:bg-gray-100"
            onClick={() => {
              setFileName(null);
              setSelectedFile(null);
              onClose();
...
export default ImportWorkLocations;
