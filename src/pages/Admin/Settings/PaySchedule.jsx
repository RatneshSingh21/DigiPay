...
const PaySchedule = () => {
  ...
  useEffect(() => {
    if (selectedPayDate && payOn !== "lastDay") {
      setPayOn("fixedDay");
      setPayDay({ value: selectedPayDate.date, label: `${selectedPayDate.date}` });
    }
  }, [selectedPayDate]);
      ...
      <div className="mx-auto p-4 bg-white rounded-md shadow">
          ...
          <div className="flex gap-1 flex-wrap">
            {days.map((day) => (
              <button
                ...
                className={`px-4 py-2 border text-sm rounded-sm ${selectedDays.includes(day)
            ...
            <div className="bg-white p-2 rounded shadow border w-full max-w-xs">
              ...
              <div className="grid grid-cols-7 gap-[2px] text-xs">
                ...
                {calendarDates.map((d) => (
                  <div
                    key={d.date}
                    className={`text-center rounded-full p-1 ${selectedPayDate?.date === d.date
        ...
        <div className="flex items-center justify-between">
          <button
            ...
            onClick={handleSave}
            disabled={loading}
            className={`bg-primary text-white px-6 py-2 rounded-md hover:bg-secondary flex items-center gap-2 ${loading ? "opacity-70 cursor-not-allowed" : ""
              }`}
          >
