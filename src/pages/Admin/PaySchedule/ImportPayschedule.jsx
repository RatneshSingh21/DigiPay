...
const ImportPayschedule = ({ onClose, fetchShifts }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        toast.success("Payschedule imported successfully!");
        setFileName(null);
  ...
  const handleDownloadSample = async () => {
    try {
      const response = await axiosInstance.get("/PaySchedule/export", {
        responseType: "blob",
      });
      ...
      link.href = url;
      link.setAttribute("download", "PaySchedule.xlsx");
...
export default ImportPayschedule;
