...
import useAuthStore from "../../../store/authStore"; // to get logged-in user info
...
const EmpAttendance = () => {
  const { user } = useAuthStore(); // Assuming your auth-store keeps logged-in user
  const [selectedDate, setSelectedDate] = useState(new Date());
  const [statusFilter, setStatusFilter] = useState({
    label: "All",
    value: "All",
  });
  const [attendanceData, setAttendanceData] = useState({}); // map of date -> status
  ...
  useEffect(() => {
    const fetchAttendance = async () => {
      try {
        ...
        // Filter only for current employee
        if (user?.userId) {
          data = data.filter((item) => item.employeeId === user.userId);
        }

        // Convert to map: yyyy-MM-dd -> status
        const mapped = {};
        data.forEach((item) => {
          const dateKey = format(new Date(item.attendanceDate), "yyyy-MM-dd");
          mapped[dateKey] = item.status || "Unknown";
        });
  ...
  const getStatus = (date) => {
    ...
    return attendanceData[key] || "Unknown";
  };
        ...
        <>
          ...
          <div className="grid grid-cols-7 gap-1">
            {filteredDays.map((day, index) => {
              ...
              // If attendance is found (not Unknown) → white text, else gray
              const textColor =
              ...
              return (
                <div
                  key={day}
                  className={`rounded-md sm:rounded-lg px-1 py-1 sm:p-2 cursor-grab text-center text-[10px] sm:text-xs flex flex-col justify-center items-center h-14 sm:h-16 ${color} ${textColor}`}
                >
